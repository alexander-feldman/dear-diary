...
test("DiaryApp wires Tali editing to mocked saving states", () => {
  assert.match(source, /aria-label=\"Tali’s editable entry\"/);
...
test("Alex entry is read-only and opens from collapsed preview or Done", () => {
  assert.match(source, /aria-label=\"Alex’s entry, read only\" readOnly/);
  assert.match(source, /aria-expanded=\{visibleAlex\}/);
...
test("memory and blank-day scope stays intentionally narrow", () => {
  ...
  assert.doesNotMatch(source, /Supabase|auth|reminder|analytics|streak|mood|tag|photo|AI/);
});
